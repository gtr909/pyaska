...
def update_data(com_no, tim, com, addr, host):
	...
	while (i < l_num):
		...
		if int(com_no) == int(no):
			#rstrip()が必要
			n.write(no + "<>" + tim + "<>" + com.rstrip() + "<>" + addr.rstrip() + "<>" + host.rstrip() + "\n")
		else:
			n.write(l[i])
		i += 1
...
def acpt_data(com):
	...
	f.write(no + "<>" + time + "<>" + com + "<>" + addr + "<>" + host[0] + "\n")
	f.close()
...
def abone_data(com_no):
	print("call abone_data()")
	
	update_data(com_no, "あぼーん", "あぼーん", "あぼーん", "あぼーん")
...
def bbs_list():
	...
	#print('<input type="hidden" name="time" value="' + ll[1] + '">')
...
if __name__ == "__main__":
	...
	if os.environ["REQUEST_METHOD"] == "GET":
		...
		if m == "update":
			...
			update_data(c, t, comm, addr, host)
		if m == "find":
